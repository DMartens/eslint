		...
		if (
			...
			if (!options.passOnUnprunedSuppressions) {
				...
				if (unusedSuppressionsCount > 0) {
					log.error(
						"There are suppressions left that do not occur anymore. To resolve this either re-run this command with `--prune-suggestions` to remove excess suppressions or `--pass-on-unpruned-suppression` to ignore this error.",
					);
